...
train <- train_rus[, -which(names(train_rus) %in% c('encounter_id', 'hospital_id', 'icu_id', 'readmission_status'))]
...
summary(train_rus_h2o)

# Check the distribution of the target
h2o.table(train_rus_h2o$diabetes_mellitus)
train_rus_h2o[["diabetes_mellitus"]]

### Encode response variable
# if the target is 0/1 and needs recoded, run this line
train_rus_h2o$diabetes_mellitus <- as.factor(train_rus_h2o$diabetes_mellitus) # encode the binary response as a factor

# check levels
h2o.levels(train_rus_h2o$diabetes_mellitus)

### Identify response and predictor variables
y <- "diabetes_mellitus"
x <- setdiff(names(train_rus_h2o), "diabetes_mellitus")
...
gbm_perf2@metrics$AUC # 0.5554055
gbm_perf3@metrics$AUC # 0.533143
gbm_perf4@metrics$AUC # 0.533143
gbm_perf5@metrics$AUC # 0.5364586
gbm_perf6@metrics$AUC # 0.5453321
...
# the stability of the estimation, before "trusting" the main mdoel. 
